...
window.onload = function(){
	...
	var ext;
	ext = gl.getExtension('OES_texture_float') || gl.getExtension('OES_texture_half_float'); if(ext == null){
    ...
    //いたいたのポリ
		...
		count++;
		ambient = hsva(count%360, 1.0, 0.8, 1.0); // パーティクルの色
		gl.enable(gl.BLEND);
		// フレームバッファのバインドを解除
    ...
    /* ------------ */
		gl.flush();
		// 加速度の調整
		if(mouseFlag){ velocity = 1.0; }else{ velocity *= 0.95; }
		// フレームバッファをフリップ
		flip = backBuffer;
